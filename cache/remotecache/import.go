...
type ResolveCacheImporterFunc func(ctx context.Context, g session.Group, attrs map[string]string) (Importer, Descriptor, error)

type Importer interface {
	Resolve(ctx context.Context, desc Descriptor, id string, w worker.Worker) (solver.CacheManager, error)
}

type DistributionSourceLabelSetter interface {
	SetDistributionSourceLabel(context.Context, digest.Digest) error
	SetDistributionSourceAnnotation(desc Descriptor) Descriptor
}
...
func (ci *contentCacheImporter) Resolve(ctx context.Context, desc Descriptor, id string, w worker.Worker) (solver.CacheManager, error) {
	...
	switch manifestType {
	case images.MediaTypeDockerSchema2ManifestList, MediaTypeImageIndex:
		var mfst Index
	...
	case images.MediaTypeDockerSchema2Manifest, MediaTypeImageManifest:
		var mfst Manifest
		if err := json.Unmarshal(dt, &mfst); err != nil {
			return nil, err
		}
...
func readBlob(ctx context.Context, provider content.Provider, desc Descriptor) ([]byte, error) {
